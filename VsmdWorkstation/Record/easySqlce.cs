...
namespace EasySqlCe
{
    ...
    public class AccessPoint
    {
        ...
        public bool CreateDataBase(bool CheckIfExists,ref string errMsg)
        {
            if (!AccessPointReady())
            {
                errMsg = "无法启动数据库！";
            ...
            if (CheckIfExists && File.Exists(FilePathAndName()))
            {
                //errMsg = "数据库已经存在！";
                return true;
            }
        ...
        /// <returns></returns>
        public bool CreateTable<T>(T TableItem, ref string errMsg) where T : class, new()
        {
        ...
        public int Update<T>(T SetItem, T WhereItem) where T : class, new()
        {
            ...
            string sqlStatement = ConstructSQLStatementUpdatesSimple(SetItem);
        ...
        public int Delete<T>(T SearchItem) where T : class, new()
        {
            ...
            SetPropertyInfosAndUniqueIdentifier(SearchItem);
            string sqlStatement = ConstructSQLStatementDeleteSimple(SearchItem);
        ...
        {
            return string.Format("DataSource='{0}'; Password='{1}';Max Database Size= 600", FilePathAndName(), password);
        }
        ...
        {
            string conn = string.Format("Data Source={0};Password={1};Max Database Size={2};", FilePathAndName(), oldPassword, maxSize);
            string OldConnectionString = conn;
           using (SqlCeConnection localConnection = new SqlCeConnection(conn))
            {
                using (SqlCeEngine engine = new SqlCeEngine(conn))
                {
                    string newConn = string.Format("Data Source={0};Password={1};Max Database Size={2};", FilePathAndName(), newPassword, maxSize);
        ...
        string ConstructSQLStatementUpdate<T>(T ObjectOfInterest) where T : class, new()
        {
            string setclause = GetSetClauseFromProperties(ObjectOfInterest);
            string whereClause = GetWhereClauseFromProperties(ObjectOfInterest);
        ...
        string ConstructSQLStatementUpdatesSimple<T>(T ObjectOfInterest) where T : class, new()
        {
            string setclause = GetSetClauseFromProperties(ObjectOfInterest);
            string whereClause = "UID = @uidWhere";
            string sqlStatement = "UPDATE " + typeof(T).Name + " SET " + setclause;
            sqlStatement += whereClause.Length == 0 ? "" : " WHERE " + whereClause;
            return sqlStatement;
        }
        ...
        string ConstructSQLStatementDeleteSimple<T>(T ObjectOfInterest) where T : class, new()
        {
            string whereClause = "UID = @uidWhere"; ;
            string sqlStatement = "DELETE FROM " + typeof(T).Name;
            sqlStatement += whereClause.Length == 0 ? "" : " WHERE " + whereClause;
        ...
        void AddParametersWithValuesFromPropertiesLike<T>
            (T ObjectOfInterest,
            SqlCeCommand Command,
            Suffix AddToParameterName) where T : class, new()
        {
            ...
            foreach (PropertyInfo propertyOfT in PropertiesOfT)
            {
                if (propertyOfT != CurrentUniqueIdentifier)
                    continue;
                {
                    object propertyValue = propertyOfT.GetValue(ObjectOfInterest, null);
                    string parameterName = "@" + propertyOfT.Name.ToLower()
